...
pub fn export_graph(
    ...
    let mut edges: Vec<(KirId, KirId, RelationshipKind)> = relationships
        .into_iter()
        .filter(|r| {
            edge_kind_ok(&r.kind)
    ...
    let degree_of = |edges: &[(KirId, KirId, RelationshipKind)]| -> HashMap<KirId, usize> {
    ...
    if opts.min_degree > 0 {
        ...
        edges.retain(|(s, t, _)| surviving.contains_key(s) && surviving.contains_key(t));
...
fn build_object_level(
    ...
    edges: Vec<(KirId, KirId, RelationshipKind)>,
    ...
    let mut kept_edges: Vec<(KirId, KirId, RelationshipKind)> = edges
        .into_iter()
        .filter(|(s, t, _)| kept_ids.contains(s) && kept_ids.contains(t))
        .collect();
    kept_edges.sort_by(|(s1, t1, k1), (s2, t2, k2)| {
        s1.0.cmp(&s2.0)
            .then(t1.0.cmp(&t2.0))
            .then(k1.to_string().cmp(&k2.to_string()))
    });
    kept_edges.sort_by_key(|(s, t, _)| {
    ...
    let rel_kind_index = sorted_unique(kept_edges.iter().map(|(_, _, k)| k.to_string()));
...
fn build_aggregate_level(
    ...
    let mut groups: BTreeMap<String, (String, usize)> = BTreeMap::new(); // id -> (label, count)
    for o in surviving.values() {
        let (gid, label) = group_of(o);
        member_of.insert(o.id, gid.clone());
        let e = groups.entry(gid).or_insert((label, 0));
        e.1 += 1;
    }

    // collapse edges: (min group, max group, kind) -> weight.
    let mut collapsed: BTreeMap<(String, String, String), usize> = BTreeMap::new();
    for (s, t, k) in &edges {
    ...
    let kind_index: Vec<String> = match opts.group_by {
        GroupBy::Kind => sorted_unique(groups.values().map(|(l, _)| l.clone())),
